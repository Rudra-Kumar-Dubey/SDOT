/*Consider the equation APPLE + LEMON = BANANA. Assume that each letter 
actually represents a digit from 0 to 9. Some conditions are imposed. 
The leftmost letter can't be zero in any word. There must be a one-to-one mapping
between letters and digits. In other words, if you choose the digit 5 for the letter E,
then all of the E's in the equation must be 5 and no other letter can be a 5. 
No digit can be repeated.*/

//This is an crypto arithematic problem so will need to use different concepts for it like permutation, recursion, 
